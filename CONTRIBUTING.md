...
| File | Responsibility |
| --- | --- |
| `src/index.ts` | stdioの17 tools: PTY 7、`agent_launch`、旧launcher 4、`agent_configure`、`agent_approval`、`claude_turn`、`claude_approval`、`diagnostics`。 |
...
- **PTYの公開面を小さく保つ。** 現行は17 tools。SSH、container、REPLは`pty_send`から入れ子で操作し、専用toolを増やさない。
