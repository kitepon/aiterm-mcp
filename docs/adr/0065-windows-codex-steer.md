...
0.37.0の実機smokeで、Codexのhook確認画面とnpm起動processの識別が入力送信前に停止した。
Windowsの`esc to go back` footerをCodex adapterへ追加し、画面上部へ出た見出しも認識する。
shell shimとnative本体の間のNodeも含めてprocessの祖先を辿り、同じ起動の候補だけをまとめる。
別祖先の候補は維持する。両方を修正前にfocused testで再現し、修正後の11試験と実機の入力待ち到達を確認した。
読取専用の独立反証でも追加の重大指摘はなかった。公開契約の`trust_project`を使い、手動承認や別harnessへの切替を不要にした。
