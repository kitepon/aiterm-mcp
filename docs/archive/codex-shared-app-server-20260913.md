...
> 2026-09-13完了。公開Aiterm 0.36.0の実行中Steer・終了後の同じtask再開を実機で確認した。
> 最終判断は[ADR 0064](../adr/0064-codex-steer-public-acceptance.md)、[受入結果](../evidence/codex-steer-public-live-20260913.md)を参照。
> 以下は検討・失敗・切替を含む時点ごとの履歴であり、途中の未完了表示を現在の状態として扱わない。
...
- [x] 公開packageを導入し、必要な再起動後に製品の親配送を実測する。
...
完全再起動後、公開setupはreadyとなり、公式binaryとDesktopの直接の親子関係・アプリ内toolも確認した。
公開Aitermの通常起動からの子の回答が、親の同じ実行中turnへ1回保存され、私自身が受信した。
`public-live-steer.json`と公開`pty_observe`でsubmitted／queue IDなしを突合済み。最後に同じ子への追加依頼で終了後再開を確認する。
時間指定で遅らせた2回目も同じ親turnへ入ったため、終了後試験には採用しない。次の独立した試験では、
子が親の公式rolloutのtask_completeを読取り専用で確認してから回答する。経過時間を完了の代用にしない。
