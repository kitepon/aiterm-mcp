...
状態: 実装済み、実機受入前。2026-09-22の「GO。他のharnessに影響のないようにCursor独自コードとして実装計画を立てよ。共通部分は活用せよ」に基づく。
受信口の判断は[ADR 0070](adr/0070-cursor-parent-hook-receiver.md)。公開と通常HOMEでの受入は未実施。完了後に本計画をarchiveする。
